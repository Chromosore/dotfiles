...
"   call Dret("MkVimball : file<".vbname."> already exists; use ! to insist")
  ...
  if a:0 >= 2
   ...
   let home= vimball#VimballHome(vbname)
  endif
...
"  call Decho("setting up vimball tab")
  ...
  if a:0 > 0
   ...
   let vimball_name = substitute(vimballfile, '\.v\(mb\|ba\)$', '', '')
   let home= vimball#VimballHome(vimball_name)
  endif
...
"   call Decho("case a:0=".a:0.": curfile<".curfile.">")
  ...
  if a:0 >= 2
   ...
   let home= vimball#VimballHome(curfile)
  endif

  if has('packages')
   let answer = input('Delete ' . home . ' ? (y/N) ')->tolower()
...
fun! vimball#VimballHome(vimball_name)
"  call Dfunc("vimball#VimballHome()")
  if exists("g:vimball_home")
   let home= g:vimball_home
  else
   if has('packages')
    " put the vimball in its own archive
    let packhomes = split(&rtp, ',')
    for packhome in packhomes + ['']
     if isdirectory(packhome) && filewritable(packhome) | break | endif
    endfor

    if packhome == ''
     let packhome = packhomes[0]
    endif

    if !isdirectory(packhome)
     if exists("g:vimball_mkdir")
    ...
    let home = packhome . '/pack/vimball/start/' . a:vimball_name
    if (has("win32") || has("win95") || has("win64") || has("win16"))
